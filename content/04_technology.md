...
A common application can be divided into two parts. The **frontend**, the applications that gets used by the end user and the **backend**, the applications that is used by the system itself. Most commonly the term frontend only refers to a website accessible through a browser, I will misuse this term for any native mobile application as both fulfill the same purpose -- to provide an interface to the end user.
...
Though electron applications are quite large and resource extensive compared to nativity compiled applications. Softwareslike Microsoft Teams, Visual Studio Code, Discord, Slack, WhatsApp Desktop and many more all rely on Electron @electron_electron. The benefit being that the application does not need to be developed for each individual operating system.

## VueJS

VueJS is a reactive frontend framework that is used to build complex web applications. VueJS like other popular frontend frameworks such as React or Svelte is component based. That means that the final application is divided up into a multitude of different `.vue` component files. Each component holds its own state and logic @delany_2020_vuejs, @you_introduction. 
