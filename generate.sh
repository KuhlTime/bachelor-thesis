...
pandoc -s -V papersize:a4 --filter pandoc-fignos --highlight-style my-one-light.theme --number-sections content.md --resource-path content --citeproc --bibliography=bibliography.bib -M reference-section-title=References --csl=style.csl --pdf-engine=xelatex --lua-filter pandoc-gls.lua -o content.pdf
